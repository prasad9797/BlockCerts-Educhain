...
class CertificateVerify extends React.Component {
  constructor(props) {
    super(props);
    ...
    setTimeout(() => {
      ...
      for (var i = 0; i < keys.length; i++) {
        if (document.getElementById(keys[i]) !== null) {
          document.getElementById(keys[i]).textContent =
            this.state.cert[keys[i]];
        }
      }
    }, 2000);
  }
...
export default withRouter(
  connect(mapStateToProps, mapDispatchToProps)(CertificateVerify)
);
